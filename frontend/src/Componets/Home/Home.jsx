    ...
    <>
      ...
      <div className="imgs flex justify-center" id='container'>
        <div className="slide flex mt-10" id='content'>
          <img className='slide_Img mx-2' src={first} alt="" />
          <img className='slide_Img mx-2' src={second} alt="" />
          <img className='slide_Img mx-2' src={third} alt="" />
          <img className='slide_Img mx-2' src={fouth} alt="" />
        </div>
      </div>
      <div className="flex justify-center mt-4">
        <button className='back mx-2' onClick={() => handleSlide('left')}>
          <i className='bi bi-chevron-left' id='sideBack'></i>
        </button>
        <button className="next mx-2" onClick={() => handleSlide('right')}>
      ...
      <div className="infoys">
        <div className="info-intern">
          ...
          <div className="categories flex flex-wrap justify-center mt-14">
            <p className="mr-2">POPULAR CATEGORIES :</p>
            {['Big Brands', 'Work From Home', 'Part-time', 'MBA', 'Engineering', 'Media', 'Design', 'Data Science'].map(category => (
              <span
                key={category}
                className={`category mr-4 ml-2 px-3 py-1 rounded cursor-pointer ${selectedCategory === category ? 'bg-blue-500 text-white' : ''}`}
        ...
        <div className="internships" id='container2'>
          <div className="internShip-Info flex flex-wrap justify-center">
            {filterInternShips.map((data, index) => (
              <div className="int-1 mt-6 mx-4 w-full sm:w-1/2 lg:w-1/3" key={index}>
        ...
        <div className="flex justify-center mt-9">
          <button className='back mx-2' onClick={() => handleSlideIntern('left')}>
            <i className='bi bi-chevron-left' id='sideBack'></i>
          </button>
          <button className="next mx-2" onClick={() => handleSlideIntern('right')}>
            <i className='bi bi-chevron-right' id='slide'></i>
          </button>
        </div>
      </div>
      ...
      <div className="analytics mt-8 flex flex-wrap justify-center items-center text-center">
        {[
          { count: '300K+', label: 'companies hiring' },
          { count: '10K+', label: 'new openings everyday' },
          { count: '21Mn+', label: 'active students' },
          { count: '600K+', label: 'learners' },
        ].map((item, index) => (
          <div className="text-block mt-5 mx-4" key={index}>
            <span className='font-bold text-6xl text-blue-600'>{item.count}</span>
            <p>{item.label}</p>
          </div>
        ))}
      </div>

      <div className="logins flex flex-col md:flex-row h-32 mt-8 justify-between items-center">
        <div className="cont">
          <p className="flex justify-center text-white text-xl items-center m-5">Empower your career with InternArea today</p>
        </div>
        <div className="log flex flex-col md:flex-row">
          <Link to="/register" id='buttons' className="flex items-center bg-white h-9 justify-center mt-4 text-white rounded-lg shadow-md hover:bg-gray-100 mx-2">
          ...
          <Link to="/register">
            <button className='btn6 mx-2'>Register</button>
          </Link>
        </div>
      </div>
    </>
